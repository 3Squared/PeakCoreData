...
class OperationTests: CoreDataTests, NSFetchedResultsControllerDelegate {
    ...
    func testBatchImportOperation() {
        ...
        for _ in 0..<numberOfInserts {
            ...
            // Create import operation with intermediates as input
            let operation = CoreDataImportOperation<TestEntityJSON>(persistentContainer: persistentContainer)
    ...
    func testBatchImportOutcomeNumbersAreCorrect() {
        ...
        operation.addResultBlock { result in
            ...
            outcome.insertedAndUpdated.forEach {
            ...
            XCTAssertEqual(outcome.deleted.count, 0)
            XCTAssertEqual(outcome.insertedAndUpdated.count, numberOfItems)
            
            XCTAssertEqual(outcome.inserted(of: TestEntity.self).count, numberOfItems / 2)
            XCTAssertEqual(outcome.updated(of: TestEntity.self).count, numberOfItems / 2)
            XCTAssertEqual(outcome.deleted(of: TestEntity.self).count, 0)

            XCTAssertEqual(outcome.inserted(of: AnotherEntity.self).count, 0)
            XCTAssertEqual(outcome.updated(of: AnotherEntity.self).count, 0)
            XCTAssertEqual(outcome.deleted(of: AnotherEntity.self).count, 0)
    ...
    func testCoreDataToIntermediateOperation() {
        ...
        let operation = CoreDataToIntermediateOperation<TestEntityJSON>(predicate: nil, persistentContainer: persistentContainer)
        operation.addResultBlock { (result) in
            let outcome = try! result.get()
...
class InsertThenDeleteOperation: CoreDataOperation, ProducesResult {
    ...
    override func performWork(in context: NSManagedObjectContext) {
        ...
        do {
            let firstChangeset = try calculateChangeset()
            try saveOperationContext()

            let toDelete = testEntities.prefix(deleteCount)
            toDelete.forEach { context.delete($0) }

            let finalChangeset = try calculateChangeset(with: firstChangeset)
            try saveOperationContext()
            output = .success(finalChangeset)
        } catch {
